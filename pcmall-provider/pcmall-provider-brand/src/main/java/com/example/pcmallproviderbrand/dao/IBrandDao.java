...
@Repository
public interface IBrandDao extends BaseMapper<BrandEntity> {
    @Select("SELECT brand.id,bname,create_time,update_time,image,is_delete " +
            "FROM brand INNER JOIN category_brand " +
            "ON brand.id = category_brand.bid WHERE cid=#{cid} AND is_delete=0;")
    List<BrandEntity> searchBrandByCid(Integer cid);
