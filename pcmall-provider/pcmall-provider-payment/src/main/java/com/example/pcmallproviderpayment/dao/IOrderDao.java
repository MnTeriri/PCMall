...
public interface IOrderDao extends BaseMapper<Order> {
    ...
    public List<Order> searchOrderList(@Param("searchValue") String searchValue, @Param("uid") String uid, @Param("type") Integer type, @Param("start") Integer start, @Param("pageSize") Integer pageSize);

    @SelectProvider(type = OrderSqlProvider.class, method = "getRecordsFilteredSql")
    public Long getRecordsFiltered(@Param("searchValue") String searchValue, @Param("uid") String uid, @Param("type") Integer type);

    /**
     * @param data 存储过程输出参数(data["result"])：1成功、-4SQL语句出错、-3购物车为空、-2商品缺货或者下架、-1商品数量不够
     */
